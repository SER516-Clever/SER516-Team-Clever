	...
	if (!location?.state) {
		return (
			<CustomModal
				...
				headerTitle="Invalid Authentication!"
				showModal={true}
			/>
		);
	}
						...
						<Stack>
							<div>
								...
								<Form>
									<div style={{ padding: '45px' }}>
										Welcome to Team Clever's SER516 Project! Metrics are
										important for any developer team to characterize, evaluate,
										predict and improve. This dashboard will help you evaluate
										your sprints and projects using different metrics. Read up
										more about the metrics
										<a href="/metricwiki"> here</a>.
										<br />
										<br />
										To utilize the dashboard, kindly input the project slug
										below and other details as required.
										<br />
									</div>
										...
										<InputGroup>
											...
											>
												<Form.Control
													...
													onChange={handleProjectSlugField}
												/>
											</FloatingLabel>
											<Dropdown onSelect={handleSelect}>
												...
												<Dropdown.Menu>
													<Dropdown.Item eventKey="Lead Time">
														Lead Time
													</Dropdown.Item>
													<Dropdown.Item eventKey="Cycle Time">
														Cycle Time
													</Dropdown.Item>
													<Dropdown.Item eventKey="Burndown Chart">
														Burndown Chart
													</Dropdown.Item>
													<Dropdown.Item eventKey="Dev Focus">
														Dev Focus
													</Dropdown.Item>
									...
									<Button
										variant="info"
										type="submit"
										className="submitButton backgroundButton"
							...
							{data?.metric === 'LEAD' && isLeadTime ? (
								<div>
									...
									<Graph
										type="Lead Time"
										apiData={data.leadTime.storiesLeadTime.userStory}
										avg={data.leadTime.storiesLeadTime.avgLeadTime}
										chartFor={'User Story'}
										title={`User Story ${selectedValue}`}
									/>
									<br />
									<Graph
										type="Lead Time"
										apiData={data.leadTime.tasksLeadTime.task}
										avg={data.leadTime.tasksLeadTime.avgLeadTime}
										chartFor={'Task'}
										title={`Task ${selectedValue}`}
									/>
								</div>
							) : null}
							{data?.metric === 'CYCLE' && isCycleTime ? (
								<div>
									...
									<Graph
										type="Cycle Time"
										apiData={data.cycleTime.storyCycleTime.story}
										avg={data.cycleTime.storyCycleTime.avgCycleTime}
										chartFor={'User Story'}
										title={`User Story ${selectedValue}`}
									/>
									<br />
									<Graph
										type="Cycle Time"
										apiData={data.cycleTime.taskCycleTime.task}
										avg={data.cycleTime.taskCycleTime.avgCycleTime}
										chartFor={'Task'}
										title={`Task ${selectedValue}`}
									/>
								</div>
							...
							{selectedValue === 'Adopted Work' && isAdopted ? (
								<SimpleBarChart
									...
									barDataKeys={['sprintTotalPoints', 'adoptedWork']}
								/>
							) : null}
							<br />
						</Stack>
