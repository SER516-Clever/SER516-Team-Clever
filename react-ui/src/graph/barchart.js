	...
	useEffect(() => {
		...
		for (const name in data) {
			...
			for (const date in data[name]) {
				let list = data[name][date].filter(
					(task) => task['inProgressDate'] !== null
				);
		...
		<div>
			...
			<ResponsiveContainer width="100%" height={600}>
				...
				>
					...
					/>
					<Bar
						dataKey={barDataKeys[0]}
						fill="#8884d8"
						activeBar={<Rectangle fill="orange" stroke="purple" />}
					/>
					<Bar
						dataKey={barDataKeys[1]}
						fill="#888234"
						activeBar={<Rectangle fill="purple" stroke="orange" />}
					/>
				</BarChart>
			</ResponsiveContainer>
		</div>
